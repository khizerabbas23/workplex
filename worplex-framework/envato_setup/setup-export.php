...
$post_types = array('attachment', 'post', 'page', 'jobs', 'candidates', 'employer', 'package', 'faq');
    ...
    foreach ($post_datas as $post_data) {
        ...
        if ($post_type == 'attachment') {
            ...
            if (is_file($file)) {
                ...
                if (function_exists('worplex_demo_data_get_path') && is_dir(worplex_demo_data_get_path('content/'))) {
                    if (!is_dir(worplex_demo_data_get_path('content/images/'))) {
                        mkdir(worplex_demo_data_get_path('content/images/'), 0755);
                    }
                    $img_dir = worplex_demo_data_get_path('content/images/');
...
foreach ($all_options as $name => $value) {
    if (stristr($name, 'worplex_framework_options') !== false) {
...
if (function_exists('worplex_demo_data_get_path')) {
    $dir = worplex_demo_data_get_path('content/');
}
