(function($) {
    $(function() {
        ...
        WORPLEX_CFS.get_field_value = {
            ...
            'loop': function(el) {
                var rows = [];
                el.find('> .cstmfield_loop > .loop_wrapper').each(function(index) {
                    rows.push(index);
                });
                return rows.join(',');
            }
        ...
        WORPLEX_CFS.is_draft = false;
        $(document).on('click', '#save-post', function() {
            WORPLEX_CFS.is_draft = true;
        });

        $('form#post').submit(function() {

            // skip validation for drafts
            if (false === WORPLEX_CFS.is_draft) {
                var passthru = true;

                // handle each validator field
                $.each(WORPLEX_CFS.field_rules, function(field_name, obj) {
                    $('.cstmfield_input .field-' + field_name).each(function() {
                        var $this = $(this);
                        ...
                        if ('object' == typeof WORPLEX_CFS.validators[validator]) {

                            // set the DOM attribute
                            $this.attr('data-validator', obj.rule);

                            // figure out the field value
                            if ('function' == typeof WORPLEX_CFS.get_field_value[type]) {
                                var val = WORPLEX_CFS.get_field_value[type]($this);
                            }
                            else {
                            ...
                            var is_valid = WORPLEX_CFS.validators[validator]['validate'](val, $this);
                            ...
                            if (!is_valid) {
                                ...
                                if ($this.parents('.cstmfield_loop_body').length > 0) {
                                    $loop = $this.parents('.cstmfield_loop_body');
                                    $loop.addClass('open');
                                    $loop.siblings('.cstmfield_loop_head').addClass('open');
                                }

                                // error can be either a string or function
                                var error_msg = WORPLEX_CFS.validators[validator]['error'];
                                ...
                                $('#cstmfield-validation-admin-notice').show();
                            }
                        }
                    });
                });
