...
class cstmfield_third_party
{
     ...
     * WPML support
     * 
     * Properly copy WORPLEX_CFS fields on WPML post duplication (requires WPML 2.6+)
     * 
     * @param int $master_id 
     * @param string $lang 
    ...
    function wpml_handler( $master_id, $lang, $post_data, $duplicate_id ) {
        $field_data = WORPLEX_CFS()->get( false, $master_id, [ 'format' => 'raw' ] );

        if ( ! empty( $field_data ) ) {
            WORPLEX_CFS()->save( $field_data, [ 'ID' => $duplicate_id ] );
        }
    }
    ...
    function pts_post_type_filter( $args ) {
        ...
        if ( 'cstmfield' == $current_screen->id ) {
    ...
    function duplicate_post($new_post_id, $post) {
        $field_data = WORPLEX_CFS()->get( false, $post->ID, [ 'format' => 'raw' ] );
        ...
        WORPLEX_CFS()->save( $field_data, $post_data );
    }
}

new cstmfield_third_party();
