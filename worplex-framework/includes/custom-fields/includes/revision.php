...
if ( defined( 'WORPLEX_CFS_REVISIONS' ) && WORPLEX_CFS_REVISIONS ) {
    new cstmfield_revision();
}

class cstmfield_revision
{
    ...
    function __construct() {
        ...
        add_filter( '_wp_post_revision_field_cstmfield_postmeta', [ $this, '_wp_post_revision_field_postmeta' ], 10, 3 );
    ...
    function _wp_post_revision_fields( $fields ) {
        $fields[ 'cstmfield_postmeta' ] = __( 'Post Meta' );
        return $fields;
    }


    /**
     * Generate the data for the "cstmfield_postmeta" variable
     * @see wp-admin/includes/ajax-actions - wp_ajax_revisions_data()
     */
    function _wp_post_revision_field_postmeta( $value, $column, $post ) {
        $output = '';
        $fields = WORPLEX_CFS()->get( false, $post->ID );
        $field_info = WORPLEX_CFS()->get_field_info( false, $post->ID );
    ...
    function check_for_changes( $default, $last_revision, $post ) {
        $revision_data = WORPLEX_CFS()->get( false, $last_revision->ID );
        $post_data = WORPLEX_CFS()->get( false, $post->ID );
    ...
    function save_post( $post_id ) {
        ...
        if ( $original_post_id ) {
            $field_data = WORPLEX_CFS()->get( false, $original_post_id, [ 'format' => 'raw' ] );
            WORPLEX_CFS()->save( $field_data, [ 'ID' => $post_id ] );
        }
    }
    ...
    function wp_restore_post_revision( $post_id, $revision_id ) {
        $field_data = WORPLEX_CFS()->get( false, $revision_id, [ 'format' => 'raw' ] );
        WORPLEX_CFS()->save( $field_data, [ 'ID' => $post_id ] );
    }
    ...
    function wp_delete_post_revision( $revision_id ) {
        ...
        $wpdb->query( "DELETE FROM {$wpdb->prefix}cstmfield_values WHERE post_id = $revision_id" );
    }
}
