...
class cstmfield_api
{
    ...
    */
    public function save_fields( $field_data = [], $post_data = [], $options = [] ) {
        ...
        if ( empty( $post_data['ID'] ) ) {
            $post_defaults = [
                'post_title'                => 'My WORPLEX_CFS post',
                'post_content'              => '',
    ...
    */
    private function save_fields_recursive( $params ) {
        ...
        if ( isset( $field_array['value'] ) || 'loop' != $field_type ) {
            ...
            $values = WORPLEX_CFS()->fields[ $field_type ]->pre_save( $values, $params['all_fields'][ $field_id ] );
            ...
            foreach ( (array) $values as $value ) {
                ...
                $wpdb->insert( $wpdb->prefix . 'cstmfield_values', $data );
                $sub_weight++;
    ...
    */
    public function get_input_fields( $params = [] ) {
        ...
        return apply_filters( 'cstmfield_get_input_fields', $output, $params );
    }
    ...
    */
    public function get_matching_groups( $params, $skip_roles = false ) {
        ...
        // Allow for overrides
        return apply_filters( 'cstmfield_matching_groups', $matches, $params, $rule_types );
    }
    ...
    */
    private function assemble_value_array( &$field_data, $hierarchy, $field, $field_value = false, $options = false ) {
        ...
        if ( false !== $field_value ) {
            ...
            $value = WORPLEX_CFS()->fields[ $field->type ]->prepare_value( $value, $field );

            if ( 'api' == $options['format'] ) {
                $value = WORPLEX_CFS()->fields[ $field->type ]->format_value_for_api( $value, $field );
            }
            elseif ( 'input' == $options['format'] ) {
                $value = WORPLEX_CFS()->fields[ $field->type ]->format_value_for_input( $value, $field );
            }
        }
    }
...
WORPLEX_CFS()->api = new cstmfield_api();
