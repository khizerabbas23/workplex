...
class cstmfield_upgrade
{
    ...
    public function __construct() {
        $this->version = WORPLEX_CFS_VERSION;
        $this->last_version = get_option('cstmfield_version');
        ...
        if ( version_compare( $this->last_version, $this->version, '<' ) ) {
            ...
            update_option( 'cstmfield_version', $this->version );
        }
    }
    ...
    private function clean_install() {
        ...
        $sql = "
        CREATE TABLE {$wpdb->prefix}cstmfield_values (
        ...
        $sql = "
        CREATE TABLE {$wpdb->prefix}cstmfield_sessions (
            id VARCHAR(32),
            data TEXT,
            expires VARCHAR(10),
            PRIMARY KEY (id)
        ...
        update_option( 'cstmfield_next_field_id', 1 );
    }
