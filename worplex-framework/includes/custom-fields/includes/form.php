...
class cstmfield_form
{
    ...
    public function init() {
        ...
        $this->session = new cstmfield_session();

        // Save the form
        if ( isset( $_POST['cstmfield']['save'] ) ) {
            if ( wp_verify_nonce( $_POST['cstmfield']['save'], 'cstmfield_save_input' ) ) {
                ...
                $field_data = isset( $_POST['cstmfield']['input'] ) ? $_POST['cstmfield']['input'] : [];
                $post_data = [];
                ...
                // Title
                if ( isset( $_POST['cstmfield']['post_title'] ) ) {
                    $post_data['post_title'] = stripslashes( $_POST['cstmfield']['post_title'] );
                }

                // Content
                if ( isset( $_POST['cstmfield']['post_content'] ) ) {
                    $post_data['post_content'] = stripslashes( $_POST['cstmfield']['post_content'] );
                }
                ...
                // Pre-save hook
                do_action( 'cstmfield_pre_save_input', $hook_params );

                // Save the input values
                $hook_params['post_data']['ID'] = WORPLEX_CFS()->save(
                    $field_data,
                    $post_data,
                    $options
                );
                ...
                // After-save hook
                do_action( 'cstmfield_after_save_input', $hook_params );
    ...
    public function load_assets() {
        ...
        wp_enqueue_script( 'cstmfield-validation', WORPLEX_CFS_URL . '/assets/js/validation.js', [ 'jquery' ], WORPLEX_CFS_VERSION );
        wp_enqueue_script( 'jquery-powertip', WORPLEX_CFS_URL . '/assets/js/jquery-powertip/jquery.powertip.min.js', [ 'jquery' ], WORPLEX_CFS_VERSION );
        wp_enqueue_style( 'jquery-powertip', WORPLEX_CFS_URL . '/assets/js/jquery-powertip/jquery.powertip.css', [], WORPLEX_CFS_VERSION );
        wp_enqueue_style( 'cstmfield-input', WORPLEX_CFS_URL . '/assets/css/input.css', [], WORPLEX_CFS_VERSION );
    }
...
WORPLEX_CFS['get_field_value'] = {};
WORPLEX_CFS['loop_buffer'] = [];
</script>
    ...
    function footer_scripts() {
        do_action( 'cstmfield_custom_validation' );
    }
    ...
    function admin_notice() {
        ...
        echo '<div class="notice notice-error" id="cstmfield-validation-admin-notice" style="display: none;"><p><strong>';
        echo __( 'One (or more) of your fields had validation errors. More information is available below.', 'worplex-frame' );
        echo '</strong></p></div>';
    }
    ...
    public function render( $params ) {
        ...
        $defaults = [
            ...
            'confirmation_url'      => '',
            'submit_label'          => __( 'Submit', 'worplex-frame' ),
            'front_end'             => true,
        ];
        ...
        if ( empty( $params['field_groups'] ) ) {
            $field_groups = WORPLEX_CFS()->api->get_matching_groups( $post_id, true );
        ...
        if ( ! empty( $field_groups ) ) {
            $input_fields = WORPLEX_CFS()->api->get_input_fields( [
                'group_id' => $field_groups
            ] );
        }

        // Hook to allow for overridden field settings
        $input_fields = apply_filters( 'cstmfield_pre_render_fields', $input_fields, $params );

        // The SESSION should contain all applicable field group IDs. Since add_meta_box only
        // passes 1 field group at a time, we use WORPLEX_CFS()->group_ids from admin_head.php
        // to store all group IDs needed for the SESSION.
        $all_group_ids = ( false === $params['front_end'] ) ? WORPLEX_CFS()->group_ids : $field_groups;
...
<div class="cstmfield_input no_box">
    ...
    ?>
        ...
        <div class="field" data-validator="required">
            ...
            <input type="text" name="cstmfield[post_title]" value="<?php echo empty( $post_id ) ? '' : esc_attr( $post->post_title ); ?>" />
        </div>
    ...
    ?>
        ...
        <div class="field">
            ...
            <textarea name="cstmfield[post_content]"><?php echo empty( $post_id ) ? '' : esc_textarea( $post->post_content ); ?></textarea>
        </div>
    ...
    <?php
        ...
        do_action( 'cstmfield_form_before_fields', $params, [
        ...
        foreach ( $input_fields as $key => $field ) {
            ...
            if ( 'tab' == $field->type && $is_first_tab ) {
                echo '<div class="cstmfield-tabs">';
                foreach ( $tabs as $key => $tab ) {
                    echo '<div class="cstmfield-tab" rel="' . $tab->name . '">' . $tab->label . '</div>';
                }
                echo '</div>';
                $is_first_tab = false;
            }
            ...
            if ( ! isset( $this->used_types[ $field->type ] ) ) {
                WORPLEX_CFS()->fields[ $field->type ]->input_head( $field );
            ...
            if ( ! empty( $validator ) ) {
                WORPLEX_CFS()->validators[ $field->name ] = [
                    'rule'  => $validator,
                    'type'  => $field->type
                ];
            }
            ...
            if ( 1 > (int) $field->parent_id ) {
                ...
                if ( 'tab' == $field->type ) {
                    ...
                    echo '<div class="cstmfield-tab-content cstmfield-tab-content-' . $field->name . '">';

					if ( ! empty( $field->notes ) ) {
						echo '<div class="cstmfield-tab-notes">' . esc_html( $field->notes ) . '</div>';
					}
                }
                else {
    ?>
        ...
        <div class="field field-<?php echo $field->name; ?>" data-type="<?php echo $field->type; ?>" data-name="<?php echo $field->name; ?>"">
            ...
            <a href="javascript:;" class="cstmfield_loop_toggle" title="<?php esc_html_e( 'Toggle row visibility', 'worplex-frame' ); ?>"></a>
            <?php endif; ?>
            ...
            <div class="cstmfield_<?php echo $field->type; ?>">
    ...
    <?php
                WORPLEX_CFS()->create_field( [
                    ...
                    'type'          => $field->type,
                    'input_name'    => "cstmfield[input][$field->id][value]",
    ...
    <?php
        ...
        do_action( 'cstmfield_form_after_fields', $params, [
            'group_ids'     => $all_group_ids,
            'input_fields'  => $input_fields
        ] );
    ?>
        ...
        <script>
        (function($) {
            WORPLEX_CFS.field_rules = WORPLEX_CFS.field_rules || {};
            $.extend( WORPLEX_CFS.field_rules, <?php echo json_encode( WORPLEX_CFS()->validators ); ?> );
        })(jQuery);
        </script>
        <input type="hidden" name="cstmfield[save]" value="<?php echo wp_create_nonce( 'cstmfield_save_input' ); ?>" />
        <input type="hidden" name="cstmfield[session_id]" value="<?php echo $this->session->session_id; ?>" />
...
WORPLEX_CFS()->form = new cstmfield_form();
