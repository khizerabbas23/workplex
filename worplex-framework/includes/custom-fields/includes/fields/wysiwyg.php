...
class cstmfield_wysiwyg extends cstmfield_field
{

    function __construct() {
        $this->name = 'wysiwyg';
        $this->label = __( 'Wysiwyg Editor', 'worplex-frame' );
            ...
            <td>
                <?php
                    WORPLEX_CFS()->create_field( [
                        ...
                        'options' => [
                            'choices' => [
                                'default' => __( 'Default', 'worplex-frame' ),
                                'none' => __( 'None (bypass filters)', 'worplex-frame' )
                            ],
                            'force_single' => true,
                        ],
    ...
    function input_head( $field = null ) {
        ...
        if ( 'true' == get_user_meta( get_current_user_id(), 'rich_editing', true ) ) {
            if ( ! is_admin() ) {
    ?>
        <div class="hidden"><?php wp_editor( '', 'cstmfieldwysi' ); ?></div>
    <?php
            }
    ?>
        <script>
        (function($) {
            ...
            $(function() {
                $(document).on('cstmfield/ready', '.cstmfield_add_field', function() {
                    $('.cstmfield_wysiwyg:not(.ready)').init_wysiwyg();
                });
                $('.cstmfield_wysiwyg').init_wysiwyg();
            ...
            $.fn.init_wysiwyg = function() {
                this.each(function() {
                    ...
                    var input_id = 'cstmfield_wysiwyg_' + wysiwyg_count;
            ...
            $('.meta-box-sortables, .cstmfield_loop').on('sortstart', function(event, ui) {
                tinyMCE.settings.wpautop = false;
                tinyMCE.settings.resize = 'vertical';
            ...
            $('.meta-box-sortables, .cstmfield_loop').on('sortstop', function(event, ui) {
                $(this).find('.wysiwyg').each(function() {
    ...
    function mce_external_plugins( $plugins ) {
        if ( version_compare( get_bloginfo( 'version' ), '3.9', '>=' ) ) {
            $plugins['code'] = WORPLEX_CFS_URL . '/assets/js/tinymce/code.min.js';
        }
        return $plugins;
    }
