...
class cstmfield_select extends cstmfield_field
{

    function __construct() {
        $this->name = 'select';
        $this->label = __( 'Select', 'worplex-frame' );
    ...
    function select2_code() {
        ...
        echo '<script src="' . WORPLEX_CFS_URL . '/assets/js/select2/select2.min.js"></script>';
        echo '<link rel="stylesheet" type="text/css" href="' . WORPLEX_CFS_URL . '/assets/js/select2/select2.css" />';
        ...
        (function($) {
            $(function() {
                $(document).on('cstmfield/ready', '.cstmfield_add_field', function() {
                    $('.cstmfield_select:not(.ready)').init_select();
                });
                $('.cstmfield_select').init_select();
            });
    ...
    ?>
        <tr class="field_option field_option_<?php echo $this->name; ?>">
            <td class="label">
                <label><?php _e( 'Choices', 'worplex-frame' ); ?></label>
                <p class="description"><?php _e( 'Enter one choice per line', 'worplex-frame' ); ?></p>
            </td>
            <td>
                <?php
                    WORPLEX_CFS()->create_field( [
                        'type' => 'textarea',
                        'input_name' => "cstmfield[fields][$key][options][choices]",
        ...
        <tr class="field_option field_option_<?php echo $this->name; ?>">
            <td class="label">
                <label><?php _e( 'Multi-select?', 'worplex-frame' ); ?></label>
            </td>
            <td>
                <?php
                    WORPLEX_CFS()->create_field( [
                        'type' => 'true_false',
                        'input_name' => "cstmfield[fields][$key][options][multiple]",
                        'input_class' => 'true_false',
                        'value' => $this->get_option( $field, 'multiple' ),
                        'options' => [ 'message' => __( 'This is a multi-select field', 'worplex-frame' ) ],
                     ] );
                ?>
            </td>
        </tr>
        <tr class="field_option field_option_<?php echo $this->name; ?>">
            <td class="label">
                <label><?php _e('Select2', 'worplex-frame'); ?></label>
            </td>
            <td>
                <?php
                    WORPLEX_CFS()->create_field( [
                        'type' => 'true_false',
                        'input_name' => "cstmfield[fields][$key][options][select2]",
                        'input_class' => 'true_false',
                        'value' => $this->get_option($field, 'select2'),
                        'options' => [ 'message' => __('Render this field with Select2', 'worplex-frame' ) ],
                    ] );
                ?>
            </td>
        </tr>
