...
class cstmfield_loop extends cstmfield_field
{

    function __construct() {
        $this->name = 'loop';
        $this->label = __( 'Loop', 'worplex-frame' );
        $this->values = [];
    }
    ...
    */
    function html( $field ) {
        ...
        $this->values = WORPLEX_CFS()->api->get_fields( $post->ID, [ 'format' => 'input' ] );
    ...
    */
    ...
    ?>
        <tr class="field_option field_option_<?php echo $this->name; ?>">
            <td class="label">
                <label><?php _e( 'Row Display', 'worplex-frame' ); ?></label>
            </td>
            <td>
                <?php
                    WORPLEX_CFS()->create_field( [
                        'type' => 'true_false',
                        'input_name' => "cstmfield[fields][$key][options][row_display]",
                        'input_class' => 'true_false',
                        'value' => $this->get_option( $field, 'row_display' ),
                        'options' => [ 'message' => __( 'Show the values by default', 'worplex-frame' ) ],
                    ] );
                ?>
            </td>
        </tr>
        <tr class="field_option field_option_<?php echo $this->name; ?>">
            <td class="label">
                <label><?php _e( 'Row Label', 'worplex-frame' ); ?></label>
            </td>
            <td>
                <?php
                    WORPLEX_CFS()->create_field( [
                        'type' => 'text',
                        'input_name' => "cstmfield[fields][$key][options][row_label]",
                        'value' => $this->get_option( $field, 'row_label', __( 'Loop Row', 'worplex-frame' ) ),
                    ] );
                ?>
            </td>
        </tr>
        <tr class="field_option field_option_<?php echo $this->name; ?>">
            <td class="label">
                <label><?php _e( 'Button Label', 'worplex-frame' ); ?></label>
            </td>
            <td>
                <?php
                    WORPLEX_CFS()->create_field( [
                        'type' => 'text',
                        'input_name' => "cstmfield[fields][$key][options][button_label]",
                        'value' => $this->get_option( $field, 'button_label', __( 'Add Row', 'worplex-frame' ) ),
                    ] );
                ?>
            </td>
        </tr>
    ...
    */
    function recursive_clone( $group_id, $field_id ) {
        ...
        // Get loop field
        $loop_field = WORPLEX_CFS()->api->get_input_fields( [
            'field_id' => $field_id
        ] );

        // Get sub-fields
        $results = WORPLEX_CFS()->api->get_input_fields( [
            'group_id' => $group_id,
            'parent_id' => $field_id
        ] );

        $row_label = $this->dynamic_label(
            $this->get_option( $loop_field[ $field_id ], 'row_label', __( 'Loop Row', 'worplex-frame' ) )
        );

        ob_start();
    ?>
        <div class="loop_wrapper">
            <div class="cstmfield_loop_head open">
                <a class="cstmfield_delete_field" href="javascript:;"></a>
                <a class="cstmfield_toggle_field" href="javascript:;"></a>
                <a class="cstmfield_insert_field" href="javascript:;"></a>
                <span class="label"><?php echo esc_attr( $row_label ); ?></span>
            </div>
            <div class="cstmfield_loop_body open">
            <?php foreach ( $results as $field ) : ?>
                ...
                <div class="field field-<?php echo $field->name; ?> cstmfield_<?php echo $field->type; ?>">
                <?php
                if ( 'loop' == $field->type ) :
                    $loop_field_ids[] = $field->id;
                ?>
                    <div class="table_footer">
                        <input type="button" class="button-primary cstmfield_add_field" value="<?php echo esc_attr( $this->get_option( $field, 'button_label', __( 'Add Row', 'worplex-frame' ) ) ); ?>" data-loop-tag="[clone][<?php echo $field->id; ?>]" data-rows="0" />
                    </div>
                <?php else : ?>
                <?php
                    WORPLEX_CFS()->create_field( [
                        'type' => $field->type,
                        'input_name' => "cstmfield[input][clone][$field->id][value][]",
    ...
    ?>
        ...
        <script>
        WORPLEX_CFS.loop_buffer[<?php echo $field_id; ?>] = <?php echo json_encode( $buffer ); ?>;
        </script>
    ...
    */
    function recursive_html( $group_id, $field_id, $parent_tag = '', $parent_weight = 0 ) {
        ...
        $row_label = $this->get_option( $loop_field[ $field_id ], 'row_label', __( 'Loop Row', 'worplex-frame' ) );
        $button_label = $this->get_option( $loop_field[ $field_id ], 'button_label', __( 'Add Row', 'worplex-frame' ) );
    ...
    ?>
        <div class="loop_wrapper">
            <div class="cstmfield_loop_head<?php echo $css_class; ?>">
                <a class="cstmfield_delete_field" href="javascript:;"></a>
                <a class="cstmfield_toggle_field" href="javascript:;"></a>
                <a class="cstmfield_insert_field" href="javascript:;"></a>
                <span class="label"><?php echo esc_attr( $this->dynamic_label( $row_label, $results, $values[ $i ] ) ); ?>&nbsp;</span>
            </div>
            <div class="cstmfield_loop_body<?php echo $css_class; ?>">
            <?php foreach ( $results as $field ) : ?>
                <label><?php echo $field->label; ?></label>
                ...
                <?php
                    $args = [
                        'type' => $field->type,
                        'input_name' => "cstmfield[input]{$parent_tag}[$i][$field->id][value][]",
                        'input_class' => $field->type,
                        'options' => $field->options,
                    ];
        ...
        <div class="table_footer">
            <input type="button" class="button-primary cstmfield_add_field" value="<?php echo esc_attr( $button_label ); ?>" data-loop-tag="<?php echo $parent_tag; ?>" data-rows="<?php echo ( $row_offset + 1 ); ?>" />
        </div>
    <?php
    }
    ...
    ?>
        ...
        (function($) {
            $(function() {
                $(document).on('click', '.cstmfield_add_field', function() {
                    var num_rows = $(this).attr('data-rows');
                    var loop_tag = $(this).attr('data-loop-tag');
                    var loop_id = loop_tag.match(/.*\[(.*?)\]/)[1];
                    var html = WORPLEX_CFS.loop_buffer[loop_id].replace(/\[clone\]/g, loop_tag + '[' + num_rows + ']');
                    $(this).attr('data-rows', parseInt(num_rows)+1);
                    $(html).insertBefore( $(this).closest('.table_footer') ).addClass('loop_wrapper_new');
                    $(this).trigger('cstmfield/ready');
                });

                $(document).on('click', '.cstmfield_insert_field', function(event) {
                    event.stopPropagation();
                    var $add_field = $('.cstmfield_add_field');
                    ...
                    $add_field.trigger('cstmfield/ready');
                });

                $(document).on('click', '.cstmfield_delete_field', function(event) {
                ...
                $(document).on('click', '.cstmfield_loop_head', function() {
                    $(this).toggleClass('open');
                    $(this).siblings('.cstmfield_loop_body').toggleClass('open');
                });

                // Hide or show all rows
                // The HTML is located in includes/form.php
                $(document).on('click', '.cstmfield_loop_toggle', function() {
                    $(this).closest('.field').find('.cstmfield_loop_head').toggleClass('open');
                    $(this).closest('.field').find('.cstmfield_loop_body').toggleClass('open');
                });

                $('.cstmfield_loop').sortable({
                    axis: 'y',
                    containment: 'parent',
                    items: '.loop_wrapper',
                    handle: '.cstmfield_loop_head',
                    update: function(event, ui) {
                        ...
                        var depth = $container.closest('.cstmfield_loop').parents('.cstmfield_loop').length;
                        var array_element = 3 + (depth * 2);

                        var counter = -1;
                        var last_index = -1;
                        $container.find('[name^="cstmfield[input]"]').each(function() {
