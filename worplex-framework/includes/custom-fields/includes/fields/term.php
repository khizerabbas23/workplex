...
class cstmfield_term extends cstmfield_field
{

    function __construct() {
        $this->name = 'term';
        $this->label = __( 'Term', 'worplex-frame' );
    }
    ...
    function html( $field ) {
        ...
        $args = apply_filters( 'cstmfield_field_term_query_args', $args, [ 'field' => $field ] );
    ...
    ?>
        <div class="filter_posts">
            <input type="text" class="cstmfield_filter_input" autocomplete="off" placeholder="<?php _e( 'Search terms', 'worplex-frame' ); ?>" />
        </div>
        ...
        <?php foreach ( $available_posts as $term ) : ?>
            ...
            <div rel="<?php echo $term->term_id; ?>"<?php echo $class; ?> title="<?php echo $term->name; ?>"><?php echo apply_filters( 'cstmfield_term_display', $term->name, $term->term_id, $field ); ?></div>
        <?php endforeach; ?>
        </div>

        <div class="selected_posts post_list">
        <?php foreach ( $selected_posts as $term ) : ?>
            <div rel="<?php echo $term->term_id; ?>"><span class="remove"></span><?php echo apply_filters( 'cstmfield_term_display', $term->name, $term->term_id, $field ); ?></div>
        <?php endforeach; ?>
        </div>
        <div class="clear"></div>
    ...
    function options_html( $key, $field ) {
        $args = [ 'public' => true ];
        $choices = apply_filters( 'cstmfield_field_term_taxonomies', get_taxonomies( $args ) );

    ?>
        <tr class="field_option field_option_<?php echo $this->name; ?>">
            <td class="label">
                <label><?php _e('Taxonomies', 'worplex-frame'); ?></label>
                <p class="description"><?php _e('Limit terms to the following taxonomies', 'worplex-frame'); ?></p>
            </td>
            <td>
                <?php
                    WORPLEX_CFS()->create_field( [
                        'type'          => 'select',
                        'input_name'    => "cstmfield[fields][$key][options][taxonomies]",
        ...
        (function($) {
            ...
            $(function() {
                $(document).on('cstmfield/ready', '.cstmfield_add_field', function() {
                    $('.cstmfield_term:not(.ready)').init_term();
                });
                $('.cstmfield_term').init_term();

                // add selected post
                $(document).on('click', '.cstmfield_term .available_posts div', function() {
                ...
                $(document).on('click', '.cstmfield_term .selected_posts .remove', function() {
                    var div = $(this).parent();
                ...
                // filter posts
                $(document).on('keyup', '.cstmfield_term .cstmfield_filter_input', function() {
                    var input = $(this).val();
                    var parent = $(this).closest('.field');
