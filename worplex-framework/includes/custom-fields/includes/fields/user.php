...
class cstmfield_user extends cstmfield_field
{

    function __construct() {
        $this->name = 'user';
        $this->label = __( 'User', 'worplex-frame' );
    }
    ...
    ?>
        <div class="filter_posts">
            <input type="text" class="cstmfield_filter_input" autocomplete="off" placeholder="<?php _e( 'Search users', 'worplex-frame' ); ?>" />
        </div>

        <div class="available_posts post_list">
        <?php foreach ( $available_users as $user ) : ?>
            <?php $class = ( isset( $selected_users[ $user->ID ] ) ) ? ' class="used"' : ''; ?>
            <div rel="<?php echo $user->ID; ?>"<?php echo $class; ?>><?php echo apply_filters( 'cstmfield_user_display', $user->user_login, $user->ID, $field ); ?></div>
        <?php endforeach; ?>
        </div>

        <div class="selected_posts post_list">
        <?php foreach ( $selected_users as $user ) : ?>
            <div rel="<?php echo $user->ID; ?>"><span class="remove"></span><?php echo apply_filters( 'cstmfield_user_display', $user->user_login, $user->ID, $field ); ?></div>
        <?php endforeach; ?>
        </div>
        <div class="clear"></div>
        ...
        (function($) {
            ...
            $(function() {
                $(document).on('cstmfield/ready', '.cstmfield_add_field', function() {
                    $('.cstmfield_user:not(.ready)').init_user();
                });
                $('.cstmfield_user').init_user();

                // add selected post
                $(document).on('click', '.cstmfield_user .available_posts div', function() {
                ...
                $(document).on('click', '.cstmfield_user .selected_posts .remove', function() {
                    var div = $(this).parent();
                ...
                // filter posts
                $(document).on('keyup', '.cstmfield_user .cstmfield_filter_input', function() {
                    var input = $(this).val();
                    var parent = $(this).closest('.field');
