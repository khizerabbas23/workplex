 ...
 */
if (!class_exists('Worplex_categories')) {

    class Worplex_categories extends WP_Widget
    {
        /**
...
foreach ($categories as $category) {
    ...
    $posts_count = isset($category->count) ? $category->count : 0;
        ...
        ?>
        <li><a href="<?php echo esc_url($term_link); ?>"><?php echo esc_html($category->name); ?> <span><?php echo $posts_count; ?></span></a></li>
        
        <?php
       
    
     $counter ++;
    if($counter == $number_of_post){
       break;
    }
...
add_action('widgets_init', function () {
    return register_widget("Worplex_categories");
});